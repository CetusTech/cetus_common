...
public class PropertiesLoader {
  ...
  public Properties load () {
    try {
      ...
      InputStream in = this.getClass().getClassLoader().getResourceAsStream( "CetusMessageService.properties" );
      properties.load( in );
    } catch ( IOException e ) {
      ConstantEJB.CETUS_MESSAGE_EJB_LOG.error( "Error cargando archivo de propiedades " + e.getMessage() );
    }
    return properties;
  }
  ...
  public String getProperty ( String prop, String vDefalult ) {
    ConstantEJB.CETUS_MESSAGE_EJB_LOG.debug( "Obteniendo propiedad prop=" + prop + " vDefalult=" + vDefalult );
    String value = "";
    try {
      value = properties.getProperty( prop, vDefalult );
      if ( value == null ) {
        System.err.println( "ERROR propiedad NULA " + prop );
        ConstantEJB.CETUS_MESSAGE_EJB_LOG.error( "ERROR propiedad NULA " + prop );
        ConstantEJB.CETUS_MESSAGE_EJB_LOG.error( "Asignando variable por defecto a la propiedad " + prop + " vDefalult=" + vDefalult );
      }
    } catch ( Exception e ) {
      ConstantEJB.CETUS_MESSAGE_EJB_LOG.error( "ERROR propiedad NULA " + prop, e );
      ConstantEJB.CETUS_MESSAGE_EJB_LOG.error( "Asignando variable por defecto a la propiedad " + prop + " vDefalult=" + vDefalult );
    }
    return value;
